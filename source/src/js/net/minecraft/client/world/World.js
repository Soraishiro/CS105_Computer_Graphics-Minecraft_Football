...
export default class World {
  ...
  onTick() {
    ...
    // Rain scheduling — anchored to absolute world.time (game-dev best practice)
    if (this.minecraft.settings && this.minecraft.settings.enableRain) {
      if (!this.isRaining) {
        if (this.time >= this.nextRainAt && this.time < this.rainEndsAt) {
          this.isRaining = true;
        }
      } else {
        if (this.time >= this.rainEndsAt) {
          this.isRaining = false;
          const delay = 12000 + Math.random() * 36000; // 0.5–2 in-game days
          this.nextRainAt = this.time + delay;
          this.rainEndsAt = this.nextRainAt + 2000 + Math.random() * 2000;
        }
      }
    } else {
      this.isRaining = false;
    ...
    if (
      lightLevel !== this.skylightSubtracted ||
      prevRainStrength !== this.rainStrength
    ) {
  ...
  updateLight(sourceType, x1, y1, z1, x2, y2, z2, notifyNeighbor = true) {
    ...
    if (notifyNeighbor) {
      ...
      for (let i = 0; i < max; i++) {
        ...
        if (
          ...
          meta.isOutsideOf(x1, y1, z1, x2, y2, z2)
        ) {
          return;
        }
      }
    ...
    if (this.lightUpdateQueue.length < 9999) {
      this.lightUpdateQueue.push(
        new MetadataChunkBlock(sourceType, x1, y1, z1, x2, y2, z2),
      );
    }
  ...
  chunkExists(chunkX, chunkZ) {
    return (
      this.chunkProvider !== null &&
      this.chunkProvider.chunkExists(chunkX, chunkZ)
    );
  }
  ...
  setBlockAt(x, y, z, type) {
    ...
    if (this.minecraft.worldRenderer) {
      ...
      if (type === 50 && oldType !== 50) {
        this.minecraft.worldRenderer.addDynamicLight(
          x,
          y,
          z,
          0xffcc55,
  ...
  getBlockAt(x, y, z) {
    ...
    return chunkSection == null
      ? 0
      : chunkSection.getBlockAt(x & 15, y & 15, z & 15);
  }
  ...
  getBlockDataAt(x, y, z) {
    ...
    return chunkSection == null
      ? 0
      : chunkSection.getBlockDataAt(x & 15, y & 15, z & 15);
  }
  ...
  rayTraceBlocks(from, to) {
    ...
    while (counter-- >= 0) {
      ...
      if (nearestX < nearestY && nearestX < nearestZ) {
        ...
        from = new Vector3(
          nearestX1,
          from.y + diffY * nearestX,
          from.z + diffZ * nearestX,
        );
      } else if (nearestY < nearestZ) {
        face = toY > y ? EnumBlockFace.BOTTOM : EnumBlockFace.TOP;
        from = new Vector3(
          from.x + diffX * nearestY,
          nearestY1,
          from.z + diffZ * nearestY,
        );
      } else {
        face = toZ > z ? EnumBlockFace.NORTH : EnumBlockFace.SOUTH;
        from = new Vector3(
          from.x + diffX * nearestZ,
          from.y + diffY * nearestZ,
          nearestZ1,
        );
      }
  ...
  getSunriseSunsetColor(partialTicks) {
    ...
    if (rotation >= -max && rotation <= max) {
      ...
      let strength = Math.pow(
        1.0 - (1.0 - Math.sin(factor * Math.PI)) * 0.99,
        2,
      );
  ...
  getLightBrightnessForEntity(entity) {
    let level = this.getTotalLightAt(
      Math.floor(entity.x),
      Math.floor(entity.y),
      Math.floor(entity.z),
    );
    if (isNaN(level)) return 0.1;
  ...
  getSkyColorByTemp(temperature) {
    ...
    return MathHelper.hsbToRgb(
      0.6222222 - temperature * 0.05,
      0.5 + temperature * 0.1,
      1.0,
    );
  }
  ...
  loadSpawnChunks() {
    ...
    for (let x = -viewDistance; x <= viewDistance; x++) {
      for (let z = -viewDistance; z <= viewDistance; z++) {
        this.getChunkAt((this.spawn.x >> 4) + x, (this.spawn.z >> 4) + z);
      }
    }
