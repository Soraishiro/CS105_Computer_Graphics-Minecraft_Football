...
export default class WorldGenerator extends Generator {
  ...
  populateChunk(chunkX, chunkZ) {
    ...
    let amount = Math.floor(
      (this.populationNoiseGenerator.perlin(absoluteX * 0.5, absoluteY * 0.5) /
        8 +
    ...
    let treeGenerator = bigTree
      ? new BigTreeGenerator(this.world, treeSeed)
      : new TreeGenerator(this.world, treeSeed);
  ...
  generateTerrain(chunkX, chunkZ, primer) {
    ...
    let noise = this.generateTerrainNoise(
      chunkX * range,
      0,
      chunkZ * range,
      sizeX,
    ...
    for (let indexX = 0; indexX < range; indexX++) {
      for (let indexZ = 0; indexZ < range; indexZ++) {
        for (let indexY = 0; indexY < 16; indexY++) {
          ...
          let noise4 =
            noise[((indexX + 1) * sizeX + (indexZ + 1)) * sizeZ + indexY];

          // Mutation noise values
          let mut1 =
            (noise[(indexX * sizeX + indexZ) * sizeZ + (indexY + 1)] - noise1) *
            sec;
          let mut2 =
            (noise[(indexX * sizeX + (indexZ + 1)) * sizeZ + (indexY + 1)] -
              noise2) *
            sec;
          let mut3 =
            (noise[((indexX + 1) * sizeX + indexZ) * sizeZ + (indexY + 1)] -
              noise3) *
            sec;
          let mut4 =
            (noise[
              ((indexX + 1) * sizeX + (indexZ + 1)) * sizeZ + (indexY + 1)
            ] -
              noise4) *
            sec;
  ...
  naturalize(chunkX, chunkZ, primer) {
    ...
    for (let x = 0; x < 16; x++) {
      for (let z = 0; z < 16; z++) {
        ...
        let sandPatchNoise =
          natureNoise1[x + z * 16] + this.random.nextFloat() * 0.2 > 0;
        let gravelPatchNoise =
          natureNoise2[x + z * 16] + this.random.nextFloat() * 0.2 > 3;
        let stonePatchNoise =
          natureNoise3[x + z * 16] / 3 + 3 + this.random.nextFloat() * 0.25;
  ...
  generateTerrainNoise(noiseX, noiseY, noiseZ, width, height, depth) {
    ...
    // Generate terrain noise
    let terrainNoise1 = this.terrainGenerator1.generateNoiseOctaves(
      noiseX,
      noiseY,
      noiseZ,
      width,
    ...
    let terrainNoise2 = this.terrainGenerator2.generateNoiseOctaves(
      noiseX,
      noiseY,
      noiseZ,
      width,
