...
export default class GuiMainMenu extends GuiScreen {
  ...
  init() {
    ...
    this.buttonList.push(
      new GuiButton('Singleplayer', this.width / 2 - 100, y, 200, 20, () => {
        this.minecraft.startSingleplayerStadium();
      }),
    );
    this.buttonList.push(
      new GuiButton('Quit Game', this.width / 2 - 100, y + 40, 200, 20, () => {
        this.minecraft.stop();
      }),
    );
  ...
  drawScreen(stack, mouseX, mouseY, partialTicks) {
    ...
    let rotationX =
      Math.sin((this.panoramaTimer + partialTicks) / 400.0) * 25.0 + 20.0;
  ...
  drawSplash(stack) {
    let f =
      1.8 -
      Math.abs(
        Math.sin(((new Date().getTime() % 1000) / 1000.0) * Math.PI * 2.0) *
          0.1,
      );
