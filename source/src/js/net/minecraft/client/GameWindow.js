...
export default class GameWindow {
  constructor(minecraft, canvasWrapperId) {
    ...
    // True once the user has successfully obtained pointer lock at least once.
    // Prevents the pause screen from appearing on the very first load.
    this._hadLock = false;
  ...
  registerDesktopListeners() {
    ...
    this.registerListener(document, 'mousedown', (event) => {
      ...
      this.mouseDownInterval = setInterval(
        (_) => this.minecraft.onMouseClicked(event.button),
        250,
      );
    ...
    this.registerListener(document, 'pointerlockchange', (event) => {
      let lastSwitchDuration = Date.now() - this.lastIngameSwitchTime;
        let currentLock = this.isCursorLockedToCanvas();
        if (
          ...
          this.updateFocusState(
            currentLock ? FocusStateType.LOCKED : FocusStateType.EXITED,
          );
        }
      });
    this.registerListener(document, 'pointerlockerror', (event) => {
      this.updateFocusState(FocusStateType.EXITED);
    });
      ...
      (event) => {
        for (let i = 0; i < event.touches.length; i++) {
          ...
          if (isRightHand) {
            ...
            let tileIndex =
              Math.floor(relX / tileSize) + Math.floor(relY / tileSize) * 3;
  ...
  updateScaleFactor() {
    ...
    for (
      scale = 1;
      wrapperWidth / (scale + 1) >= 320 && wrapperHeight / (scale + 1) >= 240;
      scale++
    ) {}
  ...
  updateFocusState(state) {
    ...
    if (prevLock !== nextLock) {
      ...
      if (nextLock) {
        // Mark that the player has successfully been in game at least once.
        this._hadLock = true;
        // Discard any stale mouse movement accumulated before lock was granted
        // to prevent camera snapping on first pointer lock acquisition.
        this.mouseMotionX = 0;
        this.mouseMotionY = 0;
      }

      // Only show pause screen if the player was genuinely LOCKED before
      // (not just during loading / startup REQUEST_LOCK that never resolved).
      if (currentScreen === null && !nextLock && prevState === FocusStateType.LOCKED && this._hadLock) {
