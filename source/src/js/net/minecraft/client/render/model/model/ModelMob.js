...
export default class ModelMob extends ModelBase {
  ...
  initModel() {
    ...
    switch (this.mobType) {
      ...
      case "lavaslime":
      case "magmacube":
        // Outer Body: 8x8x8 at (0, 0). The spawn code in Minecraft.js
        // uses "magmacube" (matches the local texture file name); the
        // legacy aliases stay for backwards-compat.
        this.parts["body"] = new ModelRenderer("body", W, H)
          .setTextureOffset(0, 0)
          .setRotationPoint(0.0, 16.0, 0.0)
  ...
  setRotationAngles(
    ...
    let motionY =
      Math.cos(limbSwing * 0.6662 + Math.PI) * 1.4 * limbSwingStrength;
    ...
    if (this.mobType === "squid") {
      for (let i = 0; i < 8; i++) {
        if (this.parts["tentacle" + i]) {
          this.parts["tentacle" + i].rotateAngleZ =
            Math.sin(timeAlive * 0.1 + i) * 0.1;
        }
      }
    }
  }
