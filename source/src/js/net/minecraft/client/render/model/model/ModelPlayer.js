import ModelRenderer from "../renderer/ModelRenderer.js";
import MathHelper from "../../../../util/MathHelper.js";
import ModelBase from "../ModelBase.js";

/**
 * Player model — supports modern 64x64 Minecraft skin format with overlays!
 */
export default class ModelPlayer extends ModelBase {

    constructor() {
        super();

        this.swingProgress = 0;
        this.hasItemInHand = false;
        this.isSneaking = false;

        const W = 64;
        const H = 64;

        // Base layers
        this.head = new ModelRenderer("head", W, H)
            .setTextureOffset(0, 0)
            .addBox(-4.0, -8.0, -4.0, 8, 8, 8);

        this.body = new ModelRenderer("body", W, H)
            .setTextureOffset(16, 16)
            .addBox(-4.0, 0.0, -2.0, 8, 12, 4);

        this.rightArm = new ModelRenderer("right_arm", W, H)
            .setTextureOffset(40, 16)
            .setRotationPoint(-5.0, 2.0, 0.0)
            .addBox(-3.0, -2.0, -2.0, 4, 12, 4);

        this.leftArm = new ModelRenderer("left_arm", W, H)
            .setTextureOffset(32, 48)
            .setRotationPoint(5.0, 2.0, 0.0)
            .addBox(-1.0, -2.0, -2.0, 4, 12, 4);

        this.rightLeg = new ModelRenderer("right_leg", W, H)
            .setTextureOffset(0, 16)
            .setRotationPoint(-2.0, 12.0, 0.0)
            .addBox(-2.0, 0.0, -2.0, 4, 12, 4);

        this.leftLeg = new ModelRenderer("left_leg", W, H)
            .setTextureOffset(16, 48)
            .setRotationPoint(2.0, 12.0, 0.0)
            .addBox(-2.0, 0.0, -2.0, 4, 12, 4);

        // Overlay layers (inflated by 0.25 to prevent z-fighting)
        this.headOverlay = new ModelRenderer("head_overlay", W, H)
            .setTextureOffset(32, 0)
            .addBox(-4.0, -8.0, -4.0, 8, 8, 8, 0.25);

        this.bodyOverlay = new ModelRenderer("body_overlay", W, H)
            .setTextureOffset(16, 32)
            .addBox(-4.0, 0.0, -2.0, 8, 12, 4, 0.25);

        this.rightArmOverlay = new ModelRenderer("right_arm_overlay", W, H)
            .setTextureOffset(40, 32)
            .setRotationPoint(-5.0, 2.0, 0.0)
            .addBox(-3.0, -2.0, -2.0, 4, 12, 4, 0.25);

        this.leftArmOverlay = new ModelRenderer("left_arm_overlay", W, H)
            .setTextureOffset(48, 48)
            .setRotationPoint(5.0, 2.0, 0.0)
            .addBox(-1.0, -2.0, -2.0, 4, 12, 4, 0.25);

        this.rightLegOverlay = new ModelRenderer("right_leg_overlay", W, H)
            .setTextureOffset(0, 32)
            .setRotationPoint(-2.0, 12.0, 0.0)
            .addBox(-2.0, 0.0, -2.0, 4, 12, 4, 0.25);

        this.leftLegOverlay = new ModelRenderer("left_leg_overlay", W, H)
            .setTextureOffset(0, 48)
            .setRotationPoint(2.0, 12.0, 0.0)
            .addBox(-2.0, 0.0, -2.0, 4, 12, 4, 0.25);
    }

    rebuild(tessellator, group) {
        super.rebuild(tessellator, group);

        // Base
        this.head.rebuild(tessellator, group);
        this.body.rebuild(tessellator, group);
        this.leftArm.rebuild(tessellator, group);
        this.rightArm.rebuild(tessellator, group);
        this.leftLeg.rebuild(tessellator, group);
        this.rightLeg.rebuild(tessellator, group);

        // Overlays
        this.headOverlay.rebuild(tessellator, group);
        this.bodyOverlay.rebuild(tessellator, group);
        this.leftArmOverlay.rebuild(tessellator, group);
        this.rightArmOverlay.rebuild(tessellator, group);
        this.leftLegOverlay.rebuild(tessellator, group);
        this.rightLegOverlay.rebuild(tessellator, group);
    }

    render(stack, limbSwing, limbSwingStrength, timeAlive, yaw, pitch, partialTicks) {
        this.setRotationAngles(stack, limbSwing, limbSwingStrength, timeAlive, yaw, pitch, partialTicks);

        this.head.render();
        this.body.render();
        this.rightArm.render();
        this.leftArm.render();
        this.rightLeg.render();
        this.leftLeg.render();

        this.headOverlay.render();
        this.bodyOverlay.render();
        this.rightArmOverlay.render();
        this.leftArmOverlay.render();
        this.rightLegOverlay.render();
        this.leftLegOverlay.render();

        super.render(stack, limbSwing, limbSwingStrength, timeAlive, yaw, pitch, partialTicks);
    }

    setRotationAngles(stack, limbSwing, limbSwingStrength, timeAlive, yaw, pitch, partialTicks) {
        this.head.rotateAngleY = MathHelper.toRadians(yaw);
        this.head.rotateAngleX = MathHelper.toRadians(pitch);
        this.headOverlay.rotateAngleY = this.head.rotateAngleY;
        this.headOverlay.rotateAngleX = this.head.rotateAngleX;

        this.rightArm.rotateAngleX = Math.cos(limbSwing * 0.6662 + Math.PI) * 2.0 * limbSwingStrength * 0.5;
        this.leftArm.rotateAngleX = Math.cos(limbSwing * 0.6662) * 2.0 * limbSwingStrength * 0.5;
        this.rightArm.rotateAngleZ = 0.0;
        this.leftArm.rotateAngleZ = 0.0;
        
        this.rightLeg.rotateAngleX = Math.cos(limbSwing * 0.6662) * 1.4 * limbSwingStrength;
        this.leftLeg.rotateAngleX = Math.cos(limbSwing * 0.6662 + Math.PI) * 1.4 * limbSwingStrength;
        this.rightLeg.rotateAngleY = 0.0;
        this.leftLeg.rotateAngleY = 0.0;

        this.rightArm.rotateAngleY = 0.0;
        this.rightArm.rotateAngleZ = 0.0;
        this.leftArm.rotateAngleY = 0.0;

        if (this.hasItemInHand) {
            this.rightArm.rotateAngleX = this.rightArm.rotateAngleX * 0.5 - (Math.PI / 10);
        }

        if (this.swingProgress > -9990.0) {
            let swingProgress = this.swingProgress;
            this.body.rotateAngleY = Math.sin(Math.sqrt(swingProgress) * Math.PI * 2.0) * 0.2;

            this.rightArm.rotationPointZ = Math.sin(this.body.rotateAngleY) * 5.0;
            this.rightArm.rotationPointX = -Math.cos(this.body.rotateAngleY) * 5.0;
            this.leftArm.rotationPointZ = -Math.sin(this.body.rotateAngleY) * 5.0;
            this.leftArm.rotationPointX = Math.cos(this.body.rotateAngleY) * 5.0;

            this.rightArm.rotateAngleY += this.body.rotateAngleY;
            this.leftArm.rotateAngleY += this.body.rotateAngleY;
            this.leftArm.rotateAngleX += this.body.rotateAngleY;

            swingProgress = 1.0 - swingProgress;
            swingProgress = swingProgress * swingProgress;
            swingProgress = swingProgress * swingProgress;
            swingProgress = 1.0 - swingProgress;

            let value1 = Math.sin(swingProgress * Math.PI);
            let value2 = Math.sin(swingProgress * Math.PI) * -(this.head.rotateAngleX - 0.7) * 0.75;

            this.rightArm.rotateAngleX = (this.rightArm.rotateAngleX - (value1 * 1.2 + value2));
            this.rightArm.rotateAngleY += this.body.rotateAngleY * 2.0;
            this.rightArm.rotateAngleZ += Math.sin(swingProgress * Math.PI) * -0.4;
        }

        if (this.isSneaking) {
            this.body.rotateAngleX = 0.5;
            this.rightArm.rotateAngleX += 0.4;
            this.leftArm.rotateAngleX += 0.4;
            this.rightLeg.rotationPointZ = 4.0;
            this.leftLeg.rotationPointZ = 4.0;
            this.rightLeg.rotationPointY = 9.0;
            this.leftLeg.rotationPointY = 9.0;
            this.head.rotationPointY = 1.0;
            stack.translateY(-0.2);
        } else {
            this.body.rotateAngleX = 0.0;
            this.rightLeg.rotationPointZ = 0.1;
            this.leftLeg.rotationPointZ = 0.1;
            this.rightLeg.rotationPointY = 12.0;
            this.leftLeg.rotationPointY = 12.0;
            this.head.rotationPointY = 0.0;
        }

        this.rightArm.rotateAngleZ += Math.cos(timeAlive * 0.09) * 0.05 + 0.05;
        this.leftArm.rotateAngleZ -= Math.cos(timeAlive * 0.09) * 0.05 + 0.05;
        this.rightArm.rotateAngleX += Math.sin(timeAlive * 0.067) * 0.05;
        this.leftArm.rotateAngleX -= Math.sin(timeAlive * 0.067) * 0.05;

        // Copy all base layer transform properties to the overlays
        this.bodyOverlay.rotateAngleX = this.body.rotateAngleX;
        this.bodyOverlay.rotateAngleY = this.body.rotateAngleY;
        this.bodyOverlay.rotateAngleZ = this.body.rotateAngleZ;
        this.bodyOverlay.rotationPointX = this.body.rotationPointX;
        this.bodyOverlay.rotationPointY = this.body.rotationPointY;
        this.bodyOverlay.rotationPointZ = this.body.rotationPointZ;

        this.rightArmOverlay.rotateAngleX = this.rightArm.rotateAngleX;
        this.rightArmOverlay.rotateAngleY = this.rightArm.rotateAngleY;
        this.rightArmOverlay.rotateAngleZ = this.rightArm.rotateAngleZ;
        this.rightArmOverlay.rotationPointX = this.rightArm.rotationPointX;
        this.rightArmOverlay.rotationPointY = this.rightArm.rotationPointY;
        this.rightArmOverlay.rotationPointZ = this.rightArm.rotationPointZ;

        this.leftArmOverlay.rotateAngleX = this.leftArm.rotateAngleX;
        this.leftArmOverlay.rotateAngleY = this.leftArm.rotateAngleY;
        this.leftArmOverlay.rotateAngleZ = this.leftArm.rotateAngleZ;
        this.leftArmOverlay.rotationPointX = this.leftArm.rotationPointX;
        this.leftArmOverlay.rotationPointY = this.leftArm.rotationPointY;
        this.leftArmOverlay.rotationPointZ = this.leftArm.rotationPointZ;

        this.rightLegOverlay.rotateAngleX = this.rightLeg.rotateAngleX;
        this.rightLegOverlay.rotateAngleY = this.rightLeg.rotateAngleY;
        this.rightLegOverlay.rotateAngleZ = this.rightLeg.rotateAngleZ;
        this.rightLegOverlay.rotationPointX = this.rightLeg.rotationPointX;
        this.rightLegOverlay.rotationPointY = this.rightLeg.rotationPointY;
        this.rightLegOverlay.rotationPointZ = this.rightLeg.rotationPointZ;

        this.leftLegOverlay.rotateAngleX = this.leftLeg.rotateAngleX;
        this.leftLegOverlay.rotateAngleY = this.leftLeg.rotateAngleY;
        this.leftLegOverlay.rotateAngleZ = this.leftLeg.rotateAngleZ;
        this.leftLegOverlay.rotationPointX = this.leftLeg.rotationPointX;
        this.leftLegOverlay.rotationPointY = this.leftLeg.rotationPointY;
        this.leftLegOverlay.rotationPointZ = this.leftLeg.rotationPointZ;

        this.headOverlay.rotationPointX = this.head.rotationPointX;
        this.headOverlay.rotationPointY = this.head.rotationPointY;
        this.headOverlay.rotationPointZ = this.head.rotationPointZ;
    }
}
