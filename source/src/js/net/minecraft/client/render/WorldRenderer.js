...
export default class WorldRenderer {
  ...
  constructor(minecraft, window) {
    ...
    this.textureTerrain = minecraft.getThreeTexture(
  ...
  onTick() {
    ...
    this.equippedProgress += MathHelper.clamp(
      (showHand ? 0.0 : 1.0) - this.equippedProgress,
      -0.4,
      0.4,
    );
    ...
    let brightnessAtPosition =
      this.minecraft.world.getLightBrightnessForEntity(player);
    let renderDistance = this.minecraft.settings.viewDistance / 32.0;
    let fogBrightness =
      brightnessAtPosition * (1.0 - renderDistance) + renderDistance;
  ...
  orientCamera(partialTicks) {
    ...
    let y =
      player.prevY +
      (player.y - player.prevY) * partialTicks +
      player.getEyeHeight();
    ...
    if (mode !== 0) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        let distanceToCollision = target.vector.distanceTo(
          new Vector3(x, y, z),
        );
    ...
    // Generate stars
    {
      ...
      for (let i = 0; i < 1500; i++) {
        ...
        let rotationY = Math.atan2(
          Math.sqrt(vectorX * vectorX + vectorZ * vectorZ),
          vectorY,
        );
  ...
  renderSky(partialTicks) {
    ...
    this.sunLight.shadow.camera.position.setFromMatrixPosition(
      this.sunLight.matrixWorld,
    );
    const targetPos = new THREE.Vector3().setFromMatrixPosition(
      this.sunLight.target.matrixWorld,
    );
    ...
    let brightness = Math.max(
      0,
      Math.min(1, Math.cos(angle * Math.PI * 2) * 2 + 0.5),
    );
  ...
  setupFog(x, z, inWater, partialTicks) {
    if (inWater) {
      ...
      let viewDistance =
        this.minecraft.settings.viewDistance * ChunkSection.SIZE;
      let viewFactor =
        1.0 -
        Math.pow(
          0.25 + (0.75 * this.minecraft.settings.viewDistance) / 32.0,
          0.25,
        );
      ...
      let brightness =
        ...
        (this.fogBrightness - this.prevFogBrightness) * partialTicks;

      let red =
        (fogColor.x + (skyColor.x - fogColor.x) * viewFactor) * brightness;
      let green =
        (fogColor.y + (skyColor.y - fogColor.y) * viewFactor) * brightness;
      let blue =
        (fogColor.z + (skyColor.z - fogColor.z) * viewFactor) * brightness;
      ...
      skyMesh.material.color.set(
        new THREE.Color(skyColor.x, skyColor.y, skyColor.z),
      );
      ...
      if (starBrightness > 0) {
        ...
        starsMesh.material.color.set(
          new THREE.Color(starBrightness, starBrightness, starBrightness),
        );
      }
      ...
      if (sunsetColor !== null) {
        ...
        sunsetMesh.material.color.set(
          new THREE.Color(sunsetColor.x, sunsetColor.y, sunsetColor.z),
        );
    ...
    for (let [index, chunk] of world.getChunkProvider().getChunks()) {
      ...
      if (distanceX < renderDistance && distanceZ < renderDistance) {
        ...
        for (let y in chunk.sections) {
          ...
          if (
            this.frustum.intersectsBox(chunkSection.boundingBox) &&
            ...
            // Queue for rebuild
            if (
              chunkSection.isModified &&
              !this.chunkSectionUpdateQueue.includes(chunkSection)
            ) {
  ...
  renderHand(partialTicks) {
    ...
    let itemId = firstPerson
      ? this.itemToRender
      : player.inventory.getItemInSelectedSlot();
    ...
    stack.rotateX(
      MathHelper.toRadians((player.rotationPitch - pitchArm) * 0.1),
    );
    ...
    if (hasItem) {
      ...
      this.translate(
        stack,
        0.7 * factor,
        -0.65 * factor - (1.0 - equipProgress) * 0.6,
        -0.9 * factor,
      );
      ...
      this.translate(
        stack,
        0.8 * factor,
        -0.75 * factor - (1.0 - equipProgress) * 0.6,
        -0.9 * factor,
      );
  ...
  bobbingAnimation(player, stack, partialTicks) {
    let walked = -(
      player.prevDistanceWalked +
      (player.distanceWalked - player.prevDistanceWalked) * partialTicks
    );
    let yaw =
    ...
    let pitch =
      ...
      (player.cameraPitch - player.prevCameraPitch) * partialTicks;
    ...
    stack.rotateX(
      MathHelper.toRadians(
        Math.abs(Math.cos(walked * Math.PI - 0.2) * yaw) * 5.0,
      ),
    );
  ...
  addSpotLight(x, y, z) {
    ...
    let spotLight = new THREE.SpotLight(
      0xffffff,
      this.minecraft.settings.spotLightIntensity,
