...
export default class SoundManager {
  ...
  create(worldRenderer) {
    ...
    this.audioLoader.load(
      "src/resources/sound/ambient/weather/rain.ogg",
      (buffer) => {
  ...
  playSound(name, x, y, z, volume, pitch) {
    ...
    if (typeof pool === "undefined") {
      ...
      sound.filters[0].frequency.setValueAtTime(
        12000 * pitch,
  ...
  initSoundtrack() {
    ...
    // Read initial volume from settings (0-100 → 0-1). Falls back to 0.5
    // if SoundManager was created before settings were attached.
    let vol = 0.5;
    if (
      this.minecraft &&
      typeof this.minecraft.settings.musicVolume === "number"
    ) {
      vol = Math.max(0, Math.min(1, this.minecraft.settings.musicVolume / 100));
    }
    this.soundtrackAudio.setVolume(vol);
  ...
  buildPlaylist() {
    const clOpener =
      "src/resources/sound/soundtrack/UEFAChampions LeagueAnthem2009-2024(stadium version).ogg";
    const plOpeners = [
    ...
    if (isPL) {
      ...
      unusedOpeners = [
        clOpener,
        ...plOpeners.filter((_, idx) => idx !== plIndex),
      ];
    } else {
      opener = clOpener;
  ...
  /** Set music volume in 0..100 range. Applied immediately if playing. */
